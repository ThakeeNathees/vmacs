...
const Theme* ThemeManager::theme = nullptr;
std::unordered_map<std::string, std::unique_ptr<Theme>> ThemeManager::themes;
std::vector<ThemeListener*> ThemeManager::listeners;
...
const UiThemeCache& Theme::GetUiEntries() const {
  return ui;
}
...
void Theme::LoadFromFile(const char* toml_file_path) {
  TODO;
}
...
void ThemeManager::Init() {
  const char* default_theme_name = "default";
  AddTheme(Theme::New(default_theme_name));
...
const Theme* ThemeManager::GetTheme(const std::string& name) {
  const auto& it = themes.find(name);
  if (it == themes.end()) return nullptr;
  return it->second.get();
}


void ThemeManager::AddTheme(std::unique_ptr<Theme> theme) {
...
void ThemeManager::SetTheme(const std::string& name) {
  ...
  if (iter == themes.end()) {
    Window::Error(std::string("No theme named '") + name + "' exists.");
    return;
  }
...
void ThemeManager::UnregisterListener(ThemeListener* listener) {
